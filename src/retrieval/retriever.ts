...
export class IntelligentRetriever {
  ...
  private cache = getRetrievalCache();
  ...
  async retrieve(query: RetrievalQuery): Promise<RetrievalResult[]> {
    ...
    const cached = this.cache.get(cacheKey) as RetrievalResult[] | undefined;
    if (cached) {
  ...
  private async hybridSearch(
    ...
    const fusedResults = this.reciprocalRankFusion(
      semanticResults,
      keywordResults,
      retrievalConfig.hybridSearch.semanticWeight,
      retrievalConfig.hybridSearch.keywordWeight,
      topK
    );
...
Return format: [1, 3, 2, ...] (just the numbers, most relevant first)`,
      ...
      const ranking: number[] = JSON.parse(rankingMatch[0]);
  ...
  async queryWithContext(
    ...
    const boostedResults = allResults.map((result) => {
      const isInContext = contextFilePaths.some(
        (path) => result.document.metadata.filePath.includes(path)
      );
      return {
        ...result,
...
// Singleton instance
let retrieverInstance: IntelligentRetriever | null = null;

export const getRetriever = (): IntelligentRetriever => {
