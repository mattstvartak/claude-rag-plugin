...
import { getPDFFetcher, type PDFContent } from '../fetchers/pdf-fetcher.js';
...
export class DocumentIngestionService {
  ...
  async ingestDirectory(
    ...
    // Build glob patterns
    const includePatterns = ingestionConfig.supportedExtensions.map(
      (ext) => `**/*${ext}`
    );
    ...
    const processPromises = uniqueFiles.map((filePath) =>
  ...
  async ingestPDFFromURL(
    ...
    try {
      ...
      const chunks = this.chunker.chunkPDFDocument(
        pdfContent.text,
        sourcePath,
        documentName,
        options.projectName,
...
export const getIngestionService = (): DocumentIngestionService => {
