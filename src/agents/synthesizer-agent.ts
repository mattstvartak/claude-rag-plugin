...
Format your response as JSON:
{
  ...
  private buildDocumentContext(documents: RetrievalResult[]): string {
    return documents
      .slice(0, 10)
      .map(
        (d, i) => `### [${i + 1}] ${d.document.metadata.filePath}
...
\`\`\``
  ...
  private extractAnalysisContext(metadata: Record<string, unknown>): string {
  ...
  private formatResponse(synthesis: SynthesisResult): string {
