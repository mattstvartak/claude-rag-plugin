...
Always provide structured, actionable insights.`;
    ...
    async process(context) {
        ...
        return {
            ...
            reasoning: `Analyzed ${context.documents.length} documents across ${Object.keys(groupedDocs).filter((k) => groupedDocs[k].length > 0).length} categories.`,
            actions: [
                {
                    type: 'analyze',
    ...
    groupDocuments(documents) {
        ...
        return {
            code: documents.filter((d) => codeExtensions.some((ext) => d.document.metadata.fileType === ext)),
            docs: documents.filter((d) => docExtensions.some((ext) => d.document.metadata.fileType === ext)),
            config: documents.filter((d) => configExtensions.some((ext) => d.document.metadata.fileType === ext)),
        };
    }
    async analyzeCode(query, documents) {
...
Format as JSON:
{
  ...
  "confidence": 0.85
    ...
    async analyzeDocs(query, documents) {
...
Format as JSON:
{
  ...
  "confidence": 0.85
    ...
    async analyzeConfig(query, documents) {
...
Format as JSON:
{
  ...
  "confidence": 0.85
    ...
    async synthesizeAnalyses(query, analyses) {
